...
# Configure emmeans globally:
# - type="response": report estimated marginal means on the natural (response)
#   scale rather than the link (log) scale, so results are directly interpretable
#   as expected reaction counts.
# - adjust="mvt": use the multivariate t distribution for multiple comparison
#   correction, which is more powerful than Bonferroni because it accounts for
#   correlations among contrasts.
# - infer=TRUE: include confidence intervals and p-values in all summaries
#   by default, avoiding the need to request them in every emmeans call.
emm_options(
...
```{python}
links = (
    DataFrame.from_(paths.nonnews, columns=["key", "name", "type"])
    .assign(sector="non-news")
    .merge(DataFrame.from_(paths.epochs))
    .groupby(["name", "sector", "epoch"])["type"]
    ...
    .reset_index()
    .assign(
        name=lambda df: df["name"].astype(str),
        epoch=lambda df: df["epoch"].astype(int),
        sector=lambda df: df["sector"].astype(str)
    )
)
```
...
(
    ...
    .merge(r.re_cond[["sector", "name", "epoch", "mu_epoch"]])
    .merge(links)
    .set_index(["sector", "name", "epoch"])
    .pipe(DataFrame)
    .sort_index()
    .to_(paths.proc / "outlets-emm-non-news.xlsx")
)
```
...
```{r}
# Create a virtual "sector" grouping dimension by mapping quality levels
# to news/non-news sectors. This allows emmeans to compute sector-level
# marginal means and news-vs-non-news contrasts from the quality-level
# model without needing to refit -- a key technique for the
...
```{r}
## Custom contrasts for comparing against epochs 0-4
cvec <- function(idx) {
    vec <- c(-rep(1/5, 5L), rep(0, 7L))
...
ax.annotate(
    ...
    xy=(mid, ypos), ha="center", fontsize=6,
...
eff_handles = [
    ...
    # Legend item for confidence intervals
    mpl.lines.Line2D(
...
```{r}
# Test the parallel trends assumption required for DiD validity:
# Under the null of parallel (equiproportional) trends, the news/non-news
# ratio of means should be constant across pre-treatment epochs. The Wald
# chi-square test checks whether DiD estimates for pre-treatment period
# transitions are jointly zero. Failure to reject supports DiD validity.
...
parallel_trends_pval <- function(E, V, idx = 1L:length(E)) {
    ...
    df <- length(E) # degrees of freedom
...
rlang::ll(
    "before 'War on News'" =
        parallel_trends_pval(E, V, 1L:py$START_WAR),
    "during 'War on News'" =
        parallel_trends_pval(E, V, (py$START_WAR+1):(py$END_WAR)),
    "after 'War on News'"  =
        parallel_trends_pval(E, V, (py$END_WAR+1):py$LAST_EPOCH)
)
```
...
table = (
    pd.concat({
        "Epochs": eff.index.to_frame().reset_index(drop=True),
        "Difference-in-difference": pd.concat([DataFrame({}, index=[0]), did], ignore_index=True),
        "Cumulative causal effect": eff.reset_index(drop=True),
    }, axis=1)
    .pipe(lambda df: df.set_index(pd.MultiIndex.from_frame(df.pop("Epochs"))))
    .sort_index()
)
num_cols = [c != "p" for c in table.columns.get_level_values(1)]
pv_cols = [c == "p" for c in table.columns.get_level_values(1)]
