...
```{r}
# Select three key epochs to capture the trajectory of algorithmic impact:
# - Epoch 4: pre-"war on news" baseline (before Meta began reducing news distribution)
# - Epoch 8: mid-treatment (during active algorithmic suppression of news)
# - Epoch 11: final observed period
...
# Difference-in-differences contrasts: the sequential changes in news engagement
# (epoch 4->8, 8->11) are compared against corresponding changes in non-news
# engagement to isolate effects attributable to news-specific algorithmic
# changes rather than platform-wide trends.
...
```{r}
rg2       <- subset(srg, epoch %in% c(4L, 11L))
emm2      <- emmeans(rg2, ~epoch * quality)
emm2_s    <- emmeans(rg2, ~epoch * sector)
con2      <- contrast(emm2, method = "consec", by = "quality")
con2_s    <- contrast(emm2_s, method = "consec", by = "sector")
con2_dd   <- contrast(con2, method = "trt.vs.ctrl", by = "contrast", adjust = "mvt")
con2_dd_s <- contrast(con2_s, method = "trt.vs.ctrl", by = "contrast", adjust = "mvt")
...
cdf2      <- .tidy(con2)
cdf2_s    <- .tidy(con2_s)
cdf2_dd   <- .tidy(con2_dd)
cdf2_dd_s <- .tidy(con2_dd_s)
```
...
contrast = (
    pd.concat(
        [
            r.cdf.drop(columns="sector")
            .rename(columns={"quality": "sector", "adj.p.value": "p.value"}),
            r.cdf_s.query("sector == 'news'")
            .rename(columns={"adj.p.value": "p.value"}),
            r.cdf2.drop(columns="sector").rename(columns={"quality": "sector"}),
            r.cdf2_s.query("sector == 'news'"),
        ]
    )
    ...
    .swaplevel(0, 1)
    .loc[["epoch8 / epoch4", "epoch11 / epoch8", "epoch11 / epoch4"]]
)
causal = (
    pd.concat(
        [
            r.cdf_dd.rename(columns={"adj.p.value": "p.value"}),
            r.cdf_dd_s,
            r.cdf2_dd.rename(columns={"adj.p.value": "p.value"}),
            r.cdf2_dd_s,
        ]
    )
    .assign(
