...
```{r}
## Custom contrasts for comparing against epochs 8 vs 4 and 11 vs 8
cvec <- function(ai, bi) {
    vec <- rep(0L, length(emm@levels$epoch))
    vec[ai] <-  1L
    vec[bi] <- -1L
    vec
}
C <- rlang::ll(
    "epoch8 - epoch4" = cvec(9L, 5L),
    "epoch11 - epoch8" = cvec(12L, 9L),
)


con    <- contrast(emm, method = C, by = "quality")
con_dd <- contrast(con, method = "trt.vs.ctrl", by = "contrast")
...
causal = (
    ...
    .assign(
        quality=lambda df: (
            df["contrast1"].str.extract(r"^([^/]+)")[0].str.strip()
        )
    )
    .set_index(["contrast", "quality"])
    .pipe(
