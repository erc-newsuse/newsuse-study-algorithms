...
```{r}
# Extract and reshape random effects from glmmTMB models by parsing the
# colon-separated grouping structure (e.g., "country:sector:name:epoch")
# into separate columns. This makes it easier to merge random effect
# estimates back to the data for computing variance-corrected EMMs.
...
links = (
    ...
    .groupby(["name", "quality", "epoch"])["type"]
...
```{python}
(
    r.obs.assign(epoch=lambda df: df["epoch"].astype(int))
    .merge(r.re_cond[["quality", "name", "epoch", "mu_epoch"]])
    .merge(links)
    .set_index(["quality", "name", "epoch"])
    .pipe(DataFrame)
    .sort_index()
    .to_(paths.proc / "outlets-emm-news.xlsx")
)
```
...
# Log-normal correction for random-effect heterogeneity (Jensen's inequality):
# When the link function is log and random effects are Gaussian,
# exp(E[log(Y)]) < E[Y] due to Jensen's inequality. Without this correction,
# estimated marginal means would underestimate the true population-level means.
# The offset adjusts for the random-effect variance: E[Y] = exp(mu + sigma^2/2).
#
...
(
    so.Plot(data, x="mid", y="reactions", color="quality")
    .add(
...
for _, row in epochs.iterrows():
    ax.annotate(
        row["epoch"], (row["mid"], 0.5), ha="center", va="center", fontsize=12, color="black",
        path_effects=[withStroke(foreground="white", linewidth=2)],
    )
...
fig.tight_layout()
fig.savefig(figpath / "epochs-timeline.pdf")
```
