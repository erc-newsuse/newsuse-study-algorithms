...
```{r}
# Custom S3 method extending broom.mixed::tidy.glmmTMB to also extract
# the dispersion component's fixed effects and variance components, which
# the default method omits. This is necessary because glmmTMB models
# separate conditional (mean) and dispersion sub-models, and publication
# tables need both.
...
```{python}
# Convert R formula notation (e.g., "qualitylow:epoch5", "(Intercept)")
# into human-readable, LaTeX-compatible labels for publication model tables.
# Interaction terms are formatted with multiplication signs, factor levels
# are extracted from prefixed names (e.g., "qualitylow" -> "low").
