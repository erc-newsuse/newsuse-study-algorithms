...
```{python}
# Visualize detected changepoints in the relative mean and dispersion signals
# of news engagement, overlaid with algorithmic and political events.
import importlib
...
fig.tight_layout()
fig.savefig(figpath / "changepoints-mean.pdf")
```
...
E["Timestamp"] = pd.to_datetime(E["Timestamp"]).dt.date
E["Lower bound"] = pd.to_datetime(E["Lower bound"]).dt.date
E["Upper bound"] = pd.to_datetime(E["Upper bound"]).dt.date
